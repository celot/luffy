...
int import_nvram(int mode, char *fname);
...
#ifdef CONFIG_CELOT//tyranno130129
#define MAX_FACTORY_CACHE_ENTRY 20

#define SERIAL_ADDR 0x1000
#define MODEL_ADDR 0x1010
#define IMEI_ADDR 0x1020
#define MODULE_SERIAL_ADDR 0x1030 // 0x1030 ~ 0x104F
#define DFLT_ADDR 0x1100
...
struct factory_nv_cache
{
	unsigned char serial[16];
	unsigned char model[16];
	cache_type cache[MAX_FACTORY_CACHE_ENTRY];
}factory_cache;
static int factory_cache_inited = 0;

extern int flash_read_factory(char *buf, off_t from, size_t len);
extern int flash_write_factory(char *buf, off_t from, size_t len);

static int  factory_nv_index(char* item)
{
	int i;
	for (i = 0; i < MAX_FACTORY_CACHE_ENTRY; i++) 
	{
		if(factory_cache.cache[i].name && !strcmp(factory_cache.cache[i].name, item))
		{
			return i;
		}
	}
...
{
	if(!factory_cache_inited)
	{
		#define READ_MAX 0xF00
		int i;
		char cache_buffer[READ_MAX];
		char *p, *q;
		factory_cache_inited = 1;

		memset(factory_cache.serial, 0, 16);
		flash_read_factory(factory_cache.serial, SERIAL_ADDR, 15);

		memset(factory_cache.model, 0, 16);
		flash_read_factory(factory_cache.model, MODEL_ADDR, 15); 

		flash_read_factory(cache_buffer, DFLT_ADDR, READ_MAX);
		p = cache_buffer;
		for (i = 0; i < MAX_FACTORY_CACHE_ENTRY; i++) 
		{
			if(NULL == (q = strchr(p, '='))) 
			{
				//prnitf("parsed failed - cannot find '='\n");
				break;
			}
			*q = '\0'; //strip '='
			factory_cache.cache[i].name = strdup(p);
			//printf("  %d '%s'->", i, p);

			p = q + 1; //value
			if (NULL == (q = strchr(p, '\0'))) 
			{
				//prnitf("parsed failed - cannot find '\\0'\n");
				break;
			}
			factory_cache.cache[i].value = strdup(p);
			//printf("'%s'\n", p);

			p = q + 1; //next entry
			if (p - cache_buffer + 1 >= READ_MAX) //end of block
				break;
			if (*p == '\0') //end of env
				break;	
		}
...
static int factory_get(int argc, char *argv[])
{
	int  factory_index = -1;

	if(argc!=2) { printf("\n"); return -1;}

	if(!factory_cache_inited) factory_init();
	if(!strncmp(argv[1],"serial", 7))
	{
		if(factory_cache.serial[0]==0xFF) printf("\n");
		else printf("%s\n", factory_cache.serial);
	}
	else if(!strncmp(argv[1],"module_model", 13))
	{
		if(factory_cache.model[0]==0xFF) printf("\n");
		else printf("%s\n", factory_cache.model);
	}
	else if(!strncmp(argv[1],"imei", 5))
	{
		unsigned char imei[16]={0,};
		flash_read_factory(imei, IMEI_ADDR, 15); 
		if(imei[0]==0xFF) printf("\n");
		else printf("%s\n", imei);
	}
	else if(!strncmp(argv[1],"module_serial", 14))
	{
		unsigned char module_serial[20]={0,};
		flash_read_factory(module_serial, MODULE_SERIAL_ADDR, 19); 
		if(module_serial[0]==0xFF) printf("\n");
		else printf("%s\n", module_serial);
	}
	else if((factory_index=factory_nv_index(argv[1]))>=0)
	{
		if(factory_index<MAX_FACTORY_CACHE_ENTRY)
		{
			printf("%s\n", factory_cache.cache[factory_index].value);
		}
		else
		{
			printf("\n");
...
void set_factory_data(char *key)
{
	int  factory_index = factory_nv_index(key);
	 if(factory_index>=0)
	 {
	 	//printf("key=%s:%s\n",key, factory_cache.cache[factory_index].value);
		if(factory_cache.cache[factory_index].value)
		{
			nvram_bufset(RT2860_NVRAM, key, factory_cache.cache[factory_index].value);
		}
	 }
}

int set_factory_data_all()
{
	int i;
	int cnt=0;
	for (i = 0; i < MAX_FACTORY_CACHE_ENTRY; i++) 
	{
		if(factory_cache.cache[i].name && factory_cache.cache[i].value)
		{
			nvram_bufset(RT2860_NVRAM, factory_cache.cache[i].name, factory_cache.cache[i].value);
			cnt++;
		}
	}
	return cnt;
...
static int factory_set(int argc, char *argv[])
{
	if(argc!=3) { printf("\n"); return -1;}

	if(!strncmp(argv[1],"imei", 5))
	{
		//if new imei not is equal to saved imei, save it..
		...
		flash_read_factory(imei, IMEI_ADDR, 15); 
		if(memcmp(imei, argv[2], len))
		{
			flash_write_factory(argv[2], IMEI_ADDR, len); 
		}
	}
	else if(!strncmp(argv[1],"module_serial", 14))
	{
		//if new iccid not is equal to saved iccid, save it..
		int len = strlen(argv[2]);
		unsigned char module_serial[20]={0,};
		len = (len>19)?19:len;

		flash_read_factory(module_serial, MODULE_SERIAL_ADDR, 19); 
		printf("factory_set  [%s]:[%s]\n", argv[2], module_serial);
		if(memcmp(module_serial, argv[2], 19))
		{
			flash_write_factory(argv[2], MODULE_SERIAL_ADDR, len); 
		}
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
...
#ifdef CONFIG_CELOT//tyranno130129
	else if (!strncmp(cmd, "factory_get", 10))
		return factory_get(argc, argv);
	else if (!strncmp(cmd, "factory_set", 10))
		return factory_set(argc, argv);
#endif //CONFIG_CELOT
	...
	defined (CONFIG_RTDEV_PLC)
		...
		else if (!strncasecmp(argv[1], "import", 7)) 
		{
			if (!strncmp(argv[2], "2860", 5) ||
			    !strncasecmp(argv[2], "rt2860", 7)) //b-compatible
				import_nvram(RT2860_NVRAM, argv[3]);
		}
		else
			usage(argv[0]);
	} else
...
#ifdef CONFIG_CELOT//tyranno121108
	factory_init();
#endif //CONFIG_CELOT
...
#ifdef CONFIG_CELOT//tyranno130129
	if(RT2860_NVRAM==mode && (set_factory_data_all()>0))
	{
		need_commit = 1;
	}
#endif //CONFIG_CELOT
