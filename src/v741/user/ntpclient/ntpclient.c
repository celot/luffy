...
///          1  ntp end
///isOk : ntp get OK
void setTimeStamp(int type, int isOk)
{
	struct  tm *disp;
	time_t dest;
	char time_stemp[256]={0,};
	char cmd[256]={0,};
	struct timeval now;
	...
	snprintf(time_stemp, 30, "%02d%02d%02d%02d%02d%02d%03d",
	         disp->tm_year + 1900 - 2000, disp->tm_mon + 1, disp->tm_mday,
	         disp->tm_hour, disp->tm_min, disp->tm_sec,now.tv_usec/1000);

	if(type==0)
	{
		//snprintf(cmd, 256, "nvram_set 2860 NTPTimeCheck %d", 1);
		snprintf(cmd, 256, "echo 1 > /var/module/NTPTimeCheck");
		system(cmd);
	}
	else if(type==1)
	{
		if(isOk==1)
		{
			//snprintf(cmd, 256, "nvram_set 2860 NTPTimeStamp %02d-%s", 11, time_stemp);
			snprintf(cmd, 256, "echo %02d-%s > /var/module/NTPTimeStamp", 11, time_stemp);
			system(cmd);
		}
		else
		{
			//snprintf(cmd, 256, "nvram_set 2860 NTPTimeStamp %02d-%s", 10, time_stemp);
			snprintf(cmd, 256, "echo %02d-%s > /var/module/NTPTimeStamp", 10, time_stemp);
			system(cmd);

			system("killall -SIGTSTP mod_man");
		}
	}

	printf("time_stemp : %s\n", time_stemp);fflush(stdout);
}
...
#define NTPQUERY_STATUS_FILE "/var/run/ntp_query_state"		// Querying : 1,   END : 0


int getNTPQueryStatus()
{
	char line[256];

	FILE* fp = fopen(NTPQUERY_STATUS_FILE, "r");		
	if(fp )
	{
...
void putNTPQueryStatus(int nState)
{
	// 파일에 저장 
	FILE* fp = fopen(NTPQUERY_STATUS_FILE, "w");
	if(fp )
	{
		fprintf(fp, "%d", nState);
		fclose(fp);
...
static int got_data_from_server = 0;
...
static void timer_handler (int signum)
{
   ...
   if (getitimer(ITIMER_REAL, &timer) == -1) return;

   
   if(timer.it_value.tv_sec ==0)
   {
	   if(getNTPQueryStatus() != 2)
		   putNTPQueryStatus(0);
	   

	
      if(got_data_from_server==0)
      {
         if (debug) printf("do module sync\n");
         system("killall -SIGTSTP mod_man");
      }
   }
   else
...
void primary_loop(int usd, int num_probes, int interval, int goodness)
{
	...
	system("echo 0 > /var/module/NTPValid");
	...
	for (;;) {
		...
		if (pack_len<0) {
			...
			system("echo 1 > /var/module/NTPValid");
			setTimeStamp(1, 1);
			putNTPQueryStatus(2);
