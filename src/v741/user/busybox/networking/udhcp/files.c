...
//#undef DEBUG
//#define DEBUG(str, args...){ printf("[DHCPD]"str"\n", ## args); fflush(stdout);}
...
void write_leases(void)
{
	...
	for (i = 0; i < server_config.max_leases; i++) {
		if (leases[i].yiaddr != 0) {
			...
			DEBUG("write_leases sizeof(leases[%d])=%d",i,sizeof(leases[i]));
	...
	while (i < server_config.max_leases
	 && full_read(fp, &lease, sizeof(lease)) == sizeof(lease)
		...
		if (y >= server_config.start_ip && y <= server_config.end_ip) {
			char hostname[16+1]={0};
			int hostlen=0;
			lease.expires = ntohl(lease.expires);
			if (!server_config.remaining)
				lease.expires -= time(NULL);
			
			hostlen = strlen(lease.hostname);			
			hostname[0] = (hostlen>15)?15:hostlen;
			memcpy(hostname+1, lease.hostname, hostname[0]);
			if (!(add_lease(hostname+1, lease.chaddr, lease.yiaddr, lease.expires))) {
