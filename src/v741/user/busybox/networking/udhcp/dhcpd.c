...
int udhcpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (1) { /* loop until universe collapses */
		...
		/*
		DEBUG("==================================");
		get_option(&packet, 0xFE);
		{
			unsigned char* temp = get_option(&packet, DHCP_HOST_NAME);
			if(temp)
			{
				unsigned char tname[16]={0,};
				unsigned char* t = packet.options;
				int i=0;
				for(i=0; i<bytes-240; i++) 
				{
					if(i>0 && i%8==0)printf(" ");
				...
				memcpy(tname, temp, 12);
				DEBUG("hostname:%s",tname);
			}
		}
		*/
