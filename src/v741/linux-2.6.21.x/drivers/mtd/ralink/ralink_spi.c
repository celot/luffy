...
int spic_init(void)
{
	...
	ra_outl(RT2880_SPICFG_REG, SPICFG_MSBFIRST | SPICFG_TXCLKEDGE_FALLING | CFG_CLK_DIV | SPICFG_SPICLKPOL | SPICFG_ADDRMODE);
#else
	ra_outl(RT2880_SPICFG_REG, SPICFG_MSBFIRST | SPICFG_TXCLKEDGE_FALLING | CFG_CLK_DIV | SPICFG_SPICLKPOL );
#endif
...
#define CFG_ERASE_SIZE 4096
static int raspi_erase_sector_4k(u32 offset)
{
	/* Wait until finished previous write command. */
	if (raspi_wait_ready(3))
	{
	...
	if (flash->chip->addr4b)
	{
		raspi_4byte_mode(1);
		raspi_write_enable();
		raspi_cmd(0x20, offset, 0, 0, 0, 0, SPIC_4B_ADDR);
		raspi_wait_sleep_ready(950);
		raspi_4byte_mode(0);
	}
	else
	{
		raspi_cmd(0x20, offset, 0, 0, 0, 0, 0);
		raspi_wait_sleep_ready(950);
	}
#else // COMMAND_MODE
#ifdef ADDRESS_4B_MODE
...
int raspi_write_cfg(loff_t to, size_t len, const u_char *buf)
{
	int ret = -1;
	size_t  wrlen;
	u32 addr,wirte_len;
	...
	addr = to;
	wirte_len = len;
	if (addr + wirte_len > flash->mtd.size)
	{
		printk("out of range...\n");
		return -EINVAL;
	}
	...
	while (wirte_len > 0) 
	{
		...
		addr += CFG_ERASE_SIZE;
		wirte_len -= CFG_ERASE_SIZE;
	}
	up(&flash->lock);
	//je = jiffies;
	//tet = je - js;
	//printk("\n-4k-End Time [%lu] - Start Time [%lu]: \nTotlal elapsed Time [%lu]\n",js,je, tet);
	
	ret = ramtd_write(&flash->mtd, to, len, &wrlen, buf);
	if (ret != 0) 
	{
		printk("write error...\n");
		return ret;
...
int raspi_read_cfg(loff_t from, size_t len, u_char *buf)
{
	int ret = -1;
	size_t  rdlen;

	from += 0x00030000;
	ret = ramtd_read(&flash->mtd, from, len, &rdlen, buf);
	if (ret != 0) 
	{
		printk("read error...\n");
		return ret;
...
EXPORT_SYMBOL(raspi_write_cfg);
EXPORT_SYMBOL(raspi_read_cfg);
