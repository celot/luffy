...
#ifdef CELOT_HUB_DEBUG//tyranno140326
static struct usb_host_config * usb_hub_config=NULL;
static struct usb_host_config * usb_hub_config2=NULL;
void print_usb_debug(void)
{
	...
	if(usb_hub_config)
	{
		...
		for (; n < nintf; ++n) 
		{
			printk("%s:cp->interface[%d]=%p,cp->interface[%d]->altsetting=%p \n",__func__, 
				n, cp->interface[n],n, cp->interface[n]->altsetting);
		}
	}
...
EXPORT_SYMBOL_GPL(print_usb_debug);
#endif
...
int usb_set_configuration(struct usb_device *dev, int configuration)
{
	...
	if (cp) {
		...
		new_interfaces = kmalloc(nintf * sizeof(*new_interfaces),
				GFP_KERNEL);
#ifdef CELOT_HUB_DEBUG//tyranno140326
		printk("%s:new_interfaces=%p\n",__func__, new_interfaces );
#endif //CELOT_HUB_DEBUG
		if (!new_interfaces) {
		...
		for (; n < nintf; ++n) {
			new_interfaces[n] = kzalloc(
					...
					GFP_KERNEL);
#ifdef CELOT_HUB_DEBUG//tyranno140326
			printk("%s:new_interfaces[%d]=%p,new_interfaces[%d]->altsetting=%p \n",__func__, n, new_interfaces[n],n, new_interfaces[n]->altsetting);
#endif //CELOT_HUB_DEBUG
			if (!new_interfaces[n]) {
				dev_err(&dev->dev, "Out of memory");
				ret = -ENOMEM;
free_interfaces:
	...
	{
		if(strcmp(dev->bus->bus_name,"rt3xxx-ohci")==0)
		{
			usb_hub_config2 = cp;
		}
		else
...
#ifdef CELOT_HUB_DEBUG//tyranno140326
		printk("%s--1:cp->interface[%d]=%p, intf->altsetting=%p\n",__func__,i, cp->interface[i], intf->altsetting );
#endif //CELOT_HUB_DEBUG
...
#ifdef CELOT_HUB_DEBUG
		printk("%s--2:cp->interface[%d]=%p, intf->altsetting=%p\n",__func__,
			i, cp->interface[i], 
			intf->altsetting );
#endif
