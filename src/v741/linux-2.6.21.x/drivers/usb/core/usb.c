				      ...
				      unsigned ifnum)
{
	...
	{
#ifdef CONFIG_CELOT	//tyranno140206
		if(config->interface[i] && config->interface[i]->altsetting)
		{
		...
		{
			printk("%d:config->interface[%d]=%p, config->interface[%d]->num_altsetting = %d, config->interface[%d]->altsetting=%p\n", 
				config->desc.bNumInterfaces, 
				i, config->interface[i], 
				i, config->interface[i]->num_altsetting, 
				i, config->interface[i]->altsetting);
		}
#else			
