...
static struct usb_device_id id_table_combined [] = {
	...
	{ USB_DEVICE(FTDI_VID, FTDI_PCON_PID) },		
...
static int get_serial_info(struct usb_serial_port * port, struct serial_struct __user * retinfo)
{
	...
	// FTDI vendor ID, product ID unyou
	if(port != NULL && port->serial != NULL && port->serial->dev != NULL)
	{
		if(port->serial->dev->descriptor.idVendor ==  FTDI_VID
			&& (port->serial->dev->descriptor.idProduct == FTDI_PCON_PID ||port->serial->dev->descriptor.idProduct == FTDI_8U232AM_PID))
		{
			tmp.reserved_char[0] = (char)(port->serial->dev->descriptor.idProduct  & 0xFF);
		}
		else
		{
			tmp.reserved_char[0] = 0;
		}
	}
