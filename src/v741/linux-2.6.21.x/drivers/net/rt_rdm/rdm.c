...
#endif
{
	...
	else if (cmd == RT_RDM_CMD_SET_BASE_SYS)
	{
		...
		//printk(KERN_DEBUG "switch register base addr to system register 0x%x\n",RALINK_SYSCTL_BASE);
	}
	else if (cmd == RT_RDM_CMD_SET_BASE_WLAN)
	{
		register_control = RDM_WIRELESS_ADDR;
		//printk(KERN_DEBUG "switch register base addr to wireless register 0x%08x\n", RDM_WIRELESS_ADDR);
	}
	else if (cmd == RT_RDM_CMD_SHOW_BASE)
	{
		//printk(KERN_DEBUG "current register base addr is 0x%08x\n", register_control);
	}
	else if (cmd == RT_RDM_CMD_SET_BASE)
	{
		register_control = (*(int *)arg);
		//printk(KERN_DEBUG "switch register base addr to 0x%08x\n", register_control);
	}
	else if (cmd == RT_RDM_CMD_READ_REG)
	{
		rtvalue = le32_to_cpu(*(volatile u32 *)((*(int *)arg)));
		//printk("rtvalue %x\n", rtvalue);
		put_user(rtvalue, (int __user *)arg);
	}
	else //RT_RDM_CMD_WRITE or RT_RDM_CMD_WRITE_SILENT
	{
		...
		*(volatile u32 *)(baseaddr + offset) = cpu_to_le32((*(int *)arg));
		if ((cmd & 0xffff) == RT_RDM_CMD_WRITE)
			printk(KERN_DEBUG "write offset 0x%x, value 0x%x\n", offset, (unsigned int)(*(int *)arg));
	}
