		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == 0x200074c8 ) {
		if(pf)
		{
			...
			ppp->stats.rx_errors = 0;
			ppp->stats.rx_bytes = 0;
			ppp->stats.tx_packets = 0;
			ppp->stats.tx_errors = 0;
...
ppp_net_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	//printk( "ppp_net_ioctl() value =%x:%x\n", cmd, SIOCGPPPSTATS);
	switch (cmd) {
	case SIOCGPPPSTATS:
	...
	case 0x8BE3:
		if (ppp)
		{
			...
			if (vj != 0)
			{
				vj->sls_o_compressed = 0;
				vj->sls_o_uncompressed = 0;
				vj->sls_o_compressed = 0;
				vj->sls_o_searches = 0;
				vj->sls_o_misses = 0;
				vj->sls_i_error = 0;
				vj->sls_i_tossed = 0;
				vj->sls_i_uncompressed = 0;
				vj->sls_i_compressed = 0;
			}
			*/
		}
		break;
