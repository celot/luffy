...
void kill_sig_workq(struct work_struct *work)
{
	...
	extern int nvram_get_wanmode_0(void);
	int wanMode0 = nvram_get_wanmode_0();

       u32 stat_curr = *((volatile u32 *)(RALINK_ETH_SW_BASE+0x80));
       if(stat_curr != stat)
       {
	   	printk("Link Status Changed(0x%x -> 0x%x)\n", stat, stat_curr);
		if(stat) isChanged=1;
       }
	...
	if(isChanged)
	{
		...
		if (IS_ERR(fp)) goto step_failsafe;
		...
		if (fp->f_op && fp->f_op->read) {
			...
			if (NULL != p) 
			{
	                  ...
	                  if (((stat&(1<<25))==0) && (stat_curr&(1<<25)))
	                  #else
	                  if (((stat&(1<<29))==0) && (stat_curr&(1<<29)))
	                  #endif
	                  {
	                        if(!wanMode0) send_sig(SIGUSR1, p, 0);
	                  }
	                  #ifdef CONFIG_WAN_AT_P0
	                  else if ((stat&(1<<25)) && ((stat_curr&(1<<25))==0))
	                  #else
	                  else if ((stat&(1<<29)) && ((stat_curr&(1<<29))==0))
	                  #endif
	                  {
	                       if(!wanMode0) send_sig(SIGUSR2, p, 0);
	                  }
...
step_failsafe:
	...
	{
		//read udhcpc pid from file, and send signal USR2,USR1 to get a new IP
		fp = filp_open("/var/run/failsafe.pid", O_RDONLY, 0);
		if (IS_ERR(fp))   goto step_end;

		if (fp->f_op && fp->f_op->read) {
                  memset(pid, 0, sizeof(pid));   			
		    if (fp->f_op->read(fp, pid, 8, &fp->f_pos) > 0) {
			//printk("failsafe_pid:%s\n", pid);	
	#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
			p = pid_task(find_get_pid(simple_strtoul(pid, NULL, 10)),  PIDTYPE_PID);
	#else
			p = find_task_by_pid(simple_strtoul(pid, NULL, 10));
...
static irqreturn_t esw_interrupt(int irq, void *dev_id)
{
	...
	if (reg_int_val & PORT_ST_CHG) {
		...
		goto out;
#else
		if(stat_curr==stat) goto out;
#endif
		//tyranno140220 schedule_work(&ei_local->kill_sig_wq);
		schedule_delayed_work(&ei_local->kill_sig_dwq, ((HZ)/3));
out:
		stat = stat_curr;
	}
...
int ei_open(struct net_device *dev)
{
	...
	*((volatile u32 *)(ESW_IMR)) &= ~(ESW_INT_ALL);
	...
	INIT_DELAYED_WORK(&ei_local->kill_sig_dwq, kill_sig_workq);
...
void rt305x_esw_init(void)
{
	...
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x0168) = 0x17;
	 ...
	 */
        ...
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x0084) = 0xf0000000; //tyranno130610 0 port¸¸ open..
