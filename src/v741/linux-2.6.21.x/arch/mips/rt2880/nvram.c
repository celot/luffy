...
#define SAFE_FREE(x) {if(x) {kfree(x); x=NULL;}}
#define MALLOC_BUFFER_SIZE ENV_BLK_SIZE

static unsigned long counter = 0;

#ifdef CONFIG_CELOT_MTD
#ifdef CONFIG_CELOT_MTD_16M
extern int ra_mtd_write_nm(char *name, loff_t to, size_t len, const u_char *buf);
extern int ra_mtd_read_nm(char *name, loff_t from, size_t len, u_char *buf);
#define FLASH_BLOCK_NUM_M FLASH_BLOCK_NUM
#define RALINK_NVRAM_MTDNAME_BACK "Config2"
#else //CONFIG_CELOT_MTD_8M
extern int raspi_write_cfg(loff_t to, size_t len, const u_char *buf);
extern int raspi_read_cfg( loff_t from, size_t len, u_char *buf);
#define CONFIG_BACK_NVRAM (WAPI_NVRAM+1)
#define FLASH_BLOCK_NUM_M (FLASH_BLOCK_NUM+1)
#endif //CONFIG_CELOT_MTD_16M
#else
extern int ra_mtd_write_nm(char *name, loff_t to, size_t len, const u_char *buf);
extern int ra_mtd_read_nm(char *name, loff_t from, size_t len, u_char *buf);
#define FLASH_BLOCK_NUM_M FLASH_BLOCK_NUM
...
static block_t fb[FLASH_BLOCK_NUM_M] =
{
...
#endif
{
	...
	case RALINK_NVRAM_IOCTL_GETALL:
		...
		if (fb[index].env && nvram_getall(index, fb[index].env->data) == 0) {
...
int __init ra_nvram_init(void)
{
	...
	down(&nvram_sem);
	init_nvram_block(RT2860_NVRAM);
	/*
	for (i = 0; i < FLASH_BLOCK_NUM_M; i++)
		init_nvram_block(i);
	*/
	up(&nvram_sem);
...
static int init_nvram_block(int index)
{
	...
	fb[index].env = (env_t *)kmalloc(fb[index].flash_max_len+MALLOC_BUFFER_SIZE, GFP_KERNEL);
	if (!fb[index].env)
		return -ENOMEM;
...
#ifdef CONFIG_CELOT_MTD_8M 	
	raspi_read_cfg(from, len, (unsigned char *)fb[index].env);
#else
	ra_mtd_read_nm(RALINK_NVRAM_MTDNAME, from, len, (unsigned char *)fb[index].env);
#endif	
	...
	{
#ifdef CONFIG_CELOT_MTD
		if(RT2860_NVRAM == index)
		{
			RANV_PRINT("0.Read from nvram backup config....\n");
#ifdef CONFIG_CELOT_MTD_16M 
			from = fb[index].flash_offset;
			len = fb[index].flash_max_len;
			ra_mtd_read_nm(RALINK_NVRAM_MTDNAME_BACK, from, len, (unsigned char *)fb[index].env);
#else //CONFIG_CELOT_MTD_8M
			from = fb[CONFIG_BACK_NVRAM].flash_offset;
			len = fb[CONFIG_BACK_NVRAM].flash_max_len;
			raspi_read_cfg(from, len, (unsigned char *)fb[index].env);
#endif			
			if (nv_crc32(0, fb[index].env->data, len-sizeof(fb[index].env->crc)) != fb[index].env->crc) 
			{
				RANV_PRINT("0-1.Bad CRC %x, ignore values in flash.\n", (unsigned int)fb[index].env->crc);
				//memset(fb[index].env->data, 0, len-sizeof(fb[index].env->crc));
				//SAFE_FREE(fb[i].env->data);
				fb[index].valid = 1;
				fb[index].dirty = 0;
				return -1;
			}
...
static void ra_nvram_exit(void)
{
	...
	for (index = 0; index < FLASH_BLOCK_NUM_M; index++) {
		if (fb[index].dirty)
			nvram_commit(index);
...
static int ra_nvram_close(int index)
{
	...
	for (i = 0; i < MAX_CACHE_ENTRY; i++) {
		if (fb[index].cache[i].name) {
			SAFE_FREE(fb[index].cache[i].name);
			fb[index].cache[i].name = NULL;
		}
		if (fb[index].cache[i].value) {
			SAFE_FREE(fb[index].cache[i].value);
...
#ifdef CONFIG_CELOT_MTD
#ifdef CONFIG_CELOT_MTD_16M	
	ra_mtd_write_nm(RALINK_NVRAM_MTDNAME, to, len, (unsigned char *)fb[index].env);
#else
	raspi_write_cfg(to, len, (unsigned char *)fb[index].env);
#endif
	if(index==RT2860_NVRAM)
	{
		//write env to flash
#ifdef CONFIG_CELOT_MTD_16M	
		ra_mtd_write_nm(RALINK_NVRAM_MTDNAME_BACK, to, len, (unsigned char *)fb[index].env);
#else
		to = fb[CONFIG_BACK_NVRAM].flash_offset;
		len = fb[CONFIG_BACK_NVRAM].flash_max_len;
		raspi_write_cfg(to, len, (unsigned char *)fb[index].env);
#endif
	}
...
int nvram_commit(int index)
{
	...
	*p = '\0'; //ending null
	...
	if(RT2860_NVRAM == index)
	{
		//write env to flash
#ifdef CONFIG_CELOT_MTD_16M
		unsigned char *config_nv_buffer = 0;
		config_nv_buffer = (unsigned char *)kmalloc(0x6000, GFP_KERNEL);
		if(config_nv_buffer)
		{
			ra_mtd_read_nm(RALINK_NVRAM_MTDNAME, 0, 0x2000, config_nv_buffer);
			memcpy(config_nv_buffer+to, (unsigned char *)fb[index].env, len);
			ra_mtd_write_nm(RALINK_NVRAM_MTDNAME_BACK, 0,0x6000, config_nv_buffer);
			kfree(config_nv_buffer);
		}
		else
		{
			ra_mtd_write_nm(RALINK_NVRAM_MTDNAME_BACK, to, len, (unsigned char *)fb[index].env);
...
int nvram_set(int index, char *name, char *value)
{
	...
	if (-1 == idx) {
		...
		SAFE_FREE(fb[index].cache[idx].value);
		fb[index].cache[idx].value = kstrdup(value, GFP_KERNEL);
	}
...
int nvram_get_wanmode_0(void)
{
	char const * wanMode = nvram_get(RT2860_NVRAM, "wanMode");
	return (!wanMode || wanMode[0]=='0')?1:0;
}
...
EXPORT_SYMBOL(nvram_get_wanmode_0);
