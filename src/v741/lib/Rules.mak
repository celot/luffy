...
check_gcc=$(shell \
	if $(CROSS)gcc $(1) -S -o /dev/null -xc /dev/null > /dev/null 2>&1; \
	then echo "$(1)"; else echo "$(2)"; fi)
check_as=$(shell \
	if $(CROSS)gcc -Wa,$(1) -Wa,-Z -c -o /dev/null -xassembler /dev/null > /dev/null 2>&1; \
	then echo "-Wa,$(1)"; fi)
...
export LDPIEFLAG:=$(shell $(CROSS)ld --help | grep -q pie && echo "-Wl,-pie")
endif
endif
...
CFLAGS+=-isystem $(shell $(CROSS)gcc -print-file-name=include)
...
LIBGCC:=$(shell $(CROSS)gcc $(LIBGCC_CFLAGS) -print-libgcc-file-name)
